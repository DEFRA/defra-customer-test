...
var makeRequest = function makeRequest(client, method, url, payload, query) {
  ...
  if (client._options.credentials && client._options.credentials.clientId && client._options.credentials.accessToken) {
...
exports.createClient = function (reference, name) {
  ...
  var Client = function Client(options) {
    ...
    if (this._options.agent) {
      ...
      this._httpAgent = isHttps ? new https.Agent(this._options.agent) : new http.Agent(this._options.agent);
    } else {
      // Use default global agent(s)...
      this._httpAgent = isHttps ? DEFAULT_AGENTS.https : DEFAULT_AGENTS.http;
    }
    ...
    if (this._options.credentials && this._options.credentials.clientId && this._options.credentials.accessToken) {
      var ext = {};
      ...
      if (_.keys(ext).length > 0) {
        this._extData = new Buffer((0, _stringify2.default)(ext)).toString('base64');
      }
    }
  };
    ...
    Client.prototype[entry.name] = function () {
      ...
      var endpoint = entry.route.replace(/<([^<>]+)>/g, function (text, arg) {
        ...
        if (index !== -1) {
          ...
          if (typeof param !== 'string' && typeof param !== 'number') {
            throw new Error('URL parameter ' + arg + ' must be a string, but ' + 'we received a: ' + (typeof param === 'undefined' ? 'undefined' : (0, _typeof3.default)(param)));
          }
      ...
      var retryRequest = function retryRequest() {
        ...
        var sendRequest = function sendRequest() {
          ...
          return makeRequest(that, entry.method, url, payload, query).then(function (res) {
            // If request was successful, accept the result
            debug('Success calling: %s, (%s retries)', entry.name, attempts - 1);
            if (monitor) {
            ...
            if (res) {
              ...
              if (attempts <= that._options.retries && res.status >= 500 && // Check if it's a 5xx error
              res.status < 600) {
            ...
            debug('Request error calling %s NOT retrying!, err: %s, JSON: %s', entry.name, err, err);
            if (monitor) {
              var d = process.hrtime(start);
      ...
      if (typeof routingKeyPattern !== 'string') {
        ...
        routingKeyPattern = entry.routingKey.map(function (key) {
          ...
          if (typeof value === 'string') {
            ...
            assert(key.multipleWords || value.indexOf('.') === -1, 'routingKey pattern \'' + value + '\' for ' + key.name + ' cannot contain dots as it does not hold multiple words');
            return value;
          }
          // Check that we haven't got an invalid value
          assert(value === null || value === undefined, 'Value: \'' + value + '\' is not supported as routingKey ' + 'pattern for ' + key.name);
  ...
  Client.prototype.buildUrl = function () {
    ...
    if (args.length !== N && (!supportsOpts || args.length !== N + 1)) {
      throw new Error('Function ' + entry.name + 'buildUrl() takes ' + (N + 1) + ' arguments, but was given ' + (args.length + 1) + ' arguments');
    }
...
exports.createTemporaryCredentials = function (options) {
  ...
  assert(options.credentials.certificate === undefined || options.credentials.certificate === null, 'temporary credentials cannot be used to make new temporary ' + 'credentials; ensure that options.credentials.certificate is null');
  ...
  assert(options.expiry.getTime() - options.start.getTime() <= 31 * 24 * 60 * 60 * 1000, 'Credentials cannot span more than 31 days');
  ...
  var accessToken = crypto.createHmac('sha256', options.credentials.accessToken).update(cert.seed).digest('base64').replace(/\+/g, '-') // Replace + with - (see RFC 4648, sec. 5)
  ...
  return {
    ...
    certificate: (0, _stringify2.default)(cert)
  };
};
