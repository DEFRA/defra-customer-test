 ...
 * }
 */
var buildPulseConnectionString = function buildPulseConnectionString(options) {
  assert(options.username, 'options.username password is required');
  assert(options.password, 'options.password is required');

  // Construct connection string
  return ['amqps://', // Ensure that we're using SSL
  options.username, ':', options.password, '@', options.hostname || 'pulse.mozilla.org', ':', 5671].join('');
};

/** Connect to AMQP server while retrying connection establishment */
var retryConnect = function retryConnect(connectionString, retries) {
 ...
 * }
 */
var PulseConnection = function PulseConnection(options) {
  assert((typeof options === 'undefined' ? 'undefined' : (0, _typeof3.default)(options)) === 'object', 'options is required');
  ...
  if (!this._connecting) {
    ...
    retryConnect(this._connectionString, 7).then(function (conn) {
      ...
      conn.on('close', function () {
        ...
        that.emit('error', new Error('Connection closed unexpectedly, likely server initiated shutdown'));
      });
 ...
 */
var PulseListener = function PulseListener(options) {
  var that = this;
  ...
  this._fake = options.credentials && options.credentials.fake || options.connection && options.connection.fake;
...
PulseListener.prototype.bind = function (binding) {
  assert(typeof binding.exchange === 'string', 'Can\'t bind to unspecified exchange!');
  assert(typeof binding.routingKeyPattern === 'string', 'routingKeyPattern is required!');
  this._bindings.push(binding);
  if (!this._fake && this._channel) {
    debug('Binding %s to %s with pattern \'%s\'', this._queueName || 'exclusive queue', binding.exchange, binding.routingKeyPattern);
    return this._channel.bindQueue(this._queueName, binding.exchange, binding.routingKeyPattern);
  } else {
...
PulseListener.prototype.connect = function () {
  ...
  var channelCreated = this._connection.connect().then(function (conn) {
    ...
    channel.on('close', function () {
      ...
      that.emit('error', new Error('Channel closed unexpectedly, likely server initiated shutdown'));
    });
  ...
  this._queueName = ['queue', // Required by pulse security model
  this._connection.namespace, // Required by pulse security model
  this._options.queueName || 'exclusive/' + slugid.v4()].join('/');
  ...
  var bindingsCreated = queueCreated.then(function () {
    ...
    return Promise.all(that._bindings.map(function (binding) {
      debug('Binding %s to %s with pattern %s', that._queueName || 'exclusive queue', binding.exchange, binding.routingKeyPattern);
      return channel.bindQueue(that._queueName, binding.exchange, binding.routingKeyPattern);
    }));
  });
...
PulseListener.prototype.fakeMessage = function (message) {
  ...
  var msg = {
    content: new Buffer((0, _stringify2.default)(message.payload), 'utf-8'),
    fields: {
    ...
    properties: {
      headers: {
        CC: message.routes.map(function (r) {
          return 'route.' + r;
        })
      }
    }
...
PulseListener.prototype._handle = function (msg) {
  ...
  // Find CC'ed routes
  if (msg.properties && msg.properties.headers && msg.properties.headers.CC instanceof Array) {
  ...
  if (routingKeyReference) {
    try {
      ...
      debug('Failed to parse routingKey: %s for %s with err: %s, as JSON: %j', message.routingKey, message.exchange, err, err, err.stack);
    }
  }
  ...
  Promise.all(this.listeners('message').map(function (handler) {
    ...
    debug('Failed to process message %j from %s with error: %s, as JSON: %j', message, message.exchange, err, err, err.stack);
    if (that._fake) {
      return;
    }
...
//# sourceMappingURL=pulselistener.js.map
