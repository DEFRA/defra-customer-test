...
module.exports = function () {
    ...
    this.defineStep(/^I call defra relationship action with (.*?)$/, { timeout: 2000000 }, async function (StatusMsg) {

        //generate token
        const token = await this.activeDirectoryAuthService.getToken();
        //console.log(token)
        await this.activeDirectoryAuthService.requestCreateRelationship(token, StatusMsg);
        
    });

    this.defineStep(/^I call defra (.*?) enrolement action with (.*?)$/, { timeout: 2000000 }, async function (Service, StatusMsg) {

        //generate token
        const token = await this.activeDirectoryAuthService.getToken();
        //console.log(token)
        await this.activeDirectoryAuthService.requestCreateEnrolement(token, Service, StatusMsg);
